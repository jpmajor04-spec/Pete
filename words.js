...
// ─── PER-USER SEEDED WORD SHUFFLE ───────────────────────────────────────────

function _getUserSeed() {
  let seed = localStorage.getItem('pete_seed');
  if (!seed) {
    seed = Date.now().toString(36) + Math.random().toString(36).slice(2);
    localStorage.setItem('pete_seed', seed);
  }
  return seed;
}

function _seedToInt(str) {
  let h = 0;
...
let _shuffledWords = null;

function getShuffledWords() {
  if (_shuffledWords) return _shuffledWords;
  const rng = _mulberry32(_seedToInt(_getUserSeed()));
  const arr = [...WORDS];
...
// Get today's word — unique order per user, one word per day
